...
class Rope(DynamicalSystem):
    ...
    def hooke_damped(self,p,v):
        ...
        for i in range(0, self.n_masses-1):
            ...
            f_spring = self.k_elastic * x + self.k_shear * x + self.k_bend * x + self.c_elastic*x_dot+ self.c_shear*x_dot + self.c_bend*x_dot  #f_spring = self.k_elastic * u + self.k_shear * u + self.k_bend *u + self.c_elastic*x_dot + self.c_shear *x_dot + self.c_bend * x_dot

            # Add the force to the particles
            f=f.at[i].set(f[i]+f_spring)
            f=f.at[i+1].set(f[i+1]-f_spring)
    ...
    def change_of_state(self,s, F_act):
        ...
        F = index_add(F, index[self.actuated_masses, :], F_act)
    ...
    def animate_cloth(self, s_history, dt, fps=60, F_history=None, force_scale=0.2, gifname=None):
        ...
        def animate(i):
            ...
            #     self.plot_arrow(ax, p[self.actuated_masses], F_history[:,j], force_scale) 
