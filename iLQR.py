...
class LQR:
    ...
    def forward_pass(self,x_up,u_up,J_prev):
        ...
        while done==0:
            ...
            if (z < 10 and z > 0.000001): #or iter==1: #line search condition
    ...
    def backward_pass(self, lam, mu, x_new,u_new):
        ...
        cu=np.zeros((len(self.constraints),self.system.control_size))
        pn=ln_x+C_x.T.dot(self.multipliers[:,self.horizon-1]+self.Iu.dot(C))
        Pn=ln_xx+C_x.T.dot(self.Iu.dot(C_x))
        ...
        for i in range(self.horizon - 1, -1, -1):
            ...
            for j in range(len(self.constraints)):
                ...
                C_x[j]=self.constraints[j].evaluate_constraint_J(x,j)
                C[j]=self.constraints[j].evaluate_constraint(x,j)
            ...
            if i+1==self.horizon:
                ...
                p=Q_x + self.K[:,:,i+1].T.dot(Q_uu).dot(self.d[:,i+1]) + self.K[:,:,i+1].T.dot(Q_u) + Q_ux.T.dot(self.d[:,i+1])
                P=Q_xx + self.K[:,:,i+1].T.dot(Q_uu).dot(self.K[:,:,i+1]) + self.K[:,:,i+1].T.dot(Q_ux) + Q_ux.T.dot(self.K[:,:,i+1])
            # print("I'm here") 
            A, B = self.system.transition_J(x,u)  #matrices A and B from dynamics
            Q_x = l_xt + (A.T).dot(p) + C_x.T.dot(self.multipliers[:,i]+self.Iu.dot(C))
            Q_u = l_ut + (B.T).dot(p) + cu.T.dot(self.multipliers[:,i]+self.Iu.dot(C))
            Q_ux = l_uxt + B.T.dot(P).dot(A)
            Q_uu = l_uut + B.T.dot(P).dot(B) + cu.T.dot(self.Iu).dot(cu)+ self.reg_factor_u * np.identity(self.system.control_size)
            Q_xx = l_xxt + A.T.dot(P).dot(A) + C_x.T.dot(self.Iu).dot(C_x)
            # print(np.linalg.eigvals(Q_uu) > 0)
            if np.all(np.linalg.eigvals(Q_uu) > 0):
                
                self.K[:,:,i]=-inv(Q_uu).dot(Q_ux)
                self.d[:,i]=-inv(Q_uu).dot(Q_u)
                self.delta_V1[i]=self.d[:,i].T.dot(Q_u)
                self.delta_V2[i]=0.5*self.d[:,i].T.dot(Q_uu).dot(self.d[:,i])
            else:
