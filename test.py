...
if __name__ == '__main__':
    ...
    system.set_cost(0.1*np.identity(system.state_size), 0.05*np.identity(system.control_size))
    Q_f = 10*np.identity(system.state_size)
    for i in range(3*system.n_masses):
        Q_f[i, i] = 150*i
    system.set_final_cost(Q_f)
    
    system.set_goal(np.ravel(system.goal_position(2,0,4.2)))
    print(system.goal_position(2,0,4.2))
    fig = plt.figure(figsize=(8, 6), dpi=100)
    ax = fig.add_subplot(111, projection='3d')
    system.plot_rope(ax,system.goal_position(2,0,4))
    #print(system.initial_position(0,0,0))
    horizon=500
    
    constraint1=SphereConstraint(system.return_position_actuated_mass(x0), 3, system)
    constraint2=BoxConstraint(6.5,3.5,0.5,-0.5,3.2,2.0,system,horizon)
